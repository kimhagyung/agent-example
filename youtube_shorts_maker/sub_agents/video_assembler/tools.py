...
async def assemble_video(tool_context: ToolContext) -> str:
    ...
    """
    temp_files = []  # 임시 파일을 위한 리스트 

    try:
        # content_planner_output 추출 
        ...
        # 모든 artifact를 나열 (in 이미지랑 나레이션 )
        existing_artifacts = await tool_context.list_artifacts()

        # Find image and audio files
        image_files = [] # 이미지 파일 이름 리스트
        audio_files = [] # 오디오 파일 이름 리스트 

        for artifact_name in existing_artifacts:
            if artifact_name.startswith("scene_") and artifact_name.endswith(
                "_image.jpeg"
            ): 
                image_files.append(artifact_name) # artifact이름이 _image.jpeg로 끝나면 image_files 리스트에 넣고 
            elif artifact_name.startswith("scene_") and artifact_name.endswith(
                "_narration.mp3"
            ):
                audio_files.append(artifact_name)  # 아니면 audio_files에 넣음 

        # 위에 image_files ,audio_files를 가져와 그 리스트들의 장면 번호에 따라 정렬
        ...
        # 각 image 파일과 audio 파일을 모두 확인하면서 실제로 파일을 메모리에 로드 한다. 
        for i, (image_name, audio_name) in enumerate(zip(image_files, audio_files)):
            ...
            image_artifact = await tool_context.load_artifact(filename=image_name) # 실제 데이터 얻기 
            if image_artifact and image_artifact.inline_data:  # 해당 데이터를 사용해서 임시 이미지 파일과 임시 오디오 파일을 만든다. 
            ...
            if audio_artifact and audio_artifact.inline_data:  #임시 오디오 
                temp_audio = tempfile.NamedTemporaryFile(suffix=".mp3", delete=False)
                temp_audio.write(audio_artifact.inline_data.data)
                temp_audio.close()
                temp_audio_paths.append(temp_audio.name)
                temp_files.append(temp_audio.name)  # 임시 파일 리스트에 넣음 
        ...
        # 임시 출력 파일 생성 , 아래는 빈파일임 
        # 빈파일을 만드는 이유는 나중에 FFmpeg를 호출해서 저 파일을 출력물로 쓸 것이기 떄문이다. 
        ...
        # 방금 생성한 파일을 열어서 artifact에 저장 
        with open(output_path, "rb") as f:
            video_data = f.read()

        # mp4 video로 artifact를 만든다. 
        artifact = types.Part(
