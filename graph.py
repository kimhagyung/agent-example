...
class SupervisorOutput(BaseModel):
    next_agent : Literal["korean_agent", "spanish_agent", "greek_agent"] # 다음 agent가 누군지 알기위해 (이 3가지 옵션이 supervisor가 줄 수있는 옵션임)
    reasoning : str 
...
def make_agent_tool(tool_name, tool_description, system_prompt, tools):
    ...
    def agent_tool(state: Annotated[dict, InjectedState]): # 이렇게 하면 agent_tool이 호출될 때 여기에 Langgraph가 현재 state를 넣음(state를 tool에다 주입)
